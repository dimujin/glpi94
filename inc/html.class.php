...
class Html {
   ...
   **/
   static function includeHeader($title = '', $sector = 'none', $item = 'none', $option = '') {
      ...
      echo "<head><title>MyCloud - ".$title."</title>";
   ...
   **/
   static function footer($keepDB = false) {
      ...
      if (!empty($foundedNewVersion) && version_compare($currentVersion, $foundedNewVersion, '<')) {
         ...
         $latest_version = "<a href='https://mycloud.kz' target='_blank' title=\""
             . __s('MyCloud internet company.')."\"> "
             . "</a>";
         echo "</td>";
      }
...
//		    $fullpath = "/".$image['filepath'];
   ...
   static function getCopyrightMessage($withVersion = true) {
      $message = "<a href=\"https://mycloud.kz/\" title=\"Powered by mycloud.kz\" class=\"copyright\">";
      $message .= "MyCloud ";
      // if required, add GLPI version (eg not for login page)
      $message .= "Copyright (C) 2018-" . 2019 . " All rights reserved".
         "</a>";
      return $message;
   }
